...
import 'dog_database.dart'; // Make sure this is your database helper file
...
List<String> dogNames = ['Buddy', 'Max', 'Bella', 'Charlie', 'Luna', 'Lucy', 'Cooper', 'Daisy', 'Bailey', 'Rocky'];
...
Dog generateRandomDog(String imageUrl, String breed) {
  ...
  return Dog(name: name, gender: gender, birthDate: birthDate, imageUrl: imageUrl, breed: breed);
}
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadDogs() async {
    final adopted = await DogDatabase.instance.getDogsByStatus('adopted');
    final giveaway = await DogDatabase.instance.getDogsByStatus('giveaway');
    setState(() {
      _adoptedDogs = adopted;
      _giveAwayDogs = giveaway;
    });
  }

  void _adoptDog(Dog dog) async {
    await DogDatabase.instance.createDog(dog, 'adopted');
    final updatedList = await DogDatabase.instance.getDogsByStatus('adopted');
    setState(() => _adoptedDogs = updatedList);
  }

  void _giveAwayDog(Dog dog) async {
    await DogDatabase.instance.updateDogStatus(dog, 'giveaway');
    final adoptedList = await DogDatabase.instance.getDogsByStatus('adopted');
    final giveAwayList = await DogDatabase.instance.getDogsByStatus('giveaway');
    setState(() {
      _adoptedDogs = adoptedList;
      _giveAwayDogs = giveAwayList;
    });
  }
...
class AdoptedDogsPage extends StatelessWidget {
  ...
  const AdoptedDogsPage({super.key, required this.adoptedDogs, required this.onDogClicked, required this.onGiveAway});
